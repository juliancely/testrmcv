sap.ui.jsview("releasemanagementcockpit.view.OverviewRelease", {

	/** Specifies the Controller belonging to this View. 
	 * In the case that it is not implemented, or that "null" is returned, this View does not have a Controller.
	 * @memberOf releasemanagementcockpit.view.OverviewRelease
	 */
	getControllerName: function () {
		return "releasemanagementcockpit.controller.OverviewRelease";
	},
	...
	createContent: function (oController) {
		...
		var oTemplateTable = new sap.ui.table.Table({
			...
			columns: [
				...
				//  Sub Status
				new sap.ui.table.Column({
					id: "overviewReleaseTable-RelstatTxt",
				...
				new sap.ui.table.Column({
					id: "overviewReleaseTable-RetrofitStatus",
				...
				new sap.ui.table.Column({
					id: "overviewReleaseTable-RetrofitStatusText",
				...
				new sap.ui.table.Column({
					id: "overviewReleaseTable-PrdImpTimestamp",
				...
				// Created At
				new sap.ui.table.Column({
					id: "overviewReleaseTable-CreatedAt",
				...
				// Changed At
				new sap.ui.table.Column({
					id: "overviewReleaseTable-ChangedAt",
		...
		var oSmartTable = new sap.ui.comp.smarttable.SmartTable("overviewReleaseTable", {
			...
			persistencyKey: "OverviewReleaseDocKey",
			items: [oTemplateTable],
			smartVariant: oSmartVariantManagement
		});
		var oTemplate = new sap.ui.table.RowAction("overviewReleaseTableRowAction", {
			items: [
		...
		toolbar.addContent(new sap.m.Button({
			id: "startRetrofitButton",
			text: "{i18n>start_retrofit}", //'Start Retrofit',
			icon: "sap-icon://repost",
			press: oController.startRetrofit
		}));
		toolbar.addContent(new sap.m.MenuButton("CutoverChecksActsButton", {
			text: "{i18n>cutover_checks}", // Cutover Checks ,
			icon: "sap-icon://survey",
			defaultAction: oController.openCutoverChecksView,
			buttonMode: sap.m.MenuButtonMode.Split,
			useDefaultActionOnly: true,
			menu: new sap.m.Menu({
				itemSelected: oController.itemSelected,
				items: [
					new sap.m.MenuItem("CutoverChecks", {
						text: "{i18n>cutover_checks}", //"Cutover Checks",
						icon: "sap-icon://survey"
					}),
					new sap.m.MenuItem("CutoverActivities", {
						text: "{i18n>cutover_activities}", //"Cutover Activities",
		...
		toolbar.addContent(new sap.m.Button({
			text: "{i18n>shift_change}", //'Shift Change',
			icon: "sap-icon://undo",
			press: oController.onShiftChangePress
		}));
		toolbar.addContent(new sap.m.Button({
		...
		toolbar.addContent(new sap.m.MenuButton({
			...
			menu: new sap.m.Menu({
				itemSelected: oController.itemSelected,
				items: [
					new sap.m.MenuItem("TasklistO", {
						text: "{i18n>tasklist}", //"Tasklist",
						icon: "sap-icon://list"
					}),
					new sap.m.MenuItem("ReleaseCycleO", {
						text: "{i18n>release_cycle}", //"Release Cycle",
						icon: "sap-icon://mirrored-task-circle"
					})
				]
		...
		var oPage = new sap.m.Page({
			title: "{i18n>overview_cr}", //"Overview Current Release",
			showHeader: false,
			content: oSmartTable,
			footer: toolbar
		});
